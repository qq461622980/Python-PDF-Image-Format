...
def is_image_too_dark(image_bytes):
    """检查图像是否太暗。"""
    img = Image.open(io.BytesIO(image_bytes))
    grayscale_img = img.convert("L")  # 转为灰度图像
    histogram = grayscale_img.histogram()
    pixels = sum(histogram)
    dark_pixels = sum(histogram[:128])  # 计算暗像素（低于中灰度）

    # 如果暗像素占比超过一定阈值，则认为图像太暗
    if dark_pixels / pixels > 0.95:
        return True
    return False
...
def process_image_on_page(page_num, xref, image_bytes, img_rect, brightness_threshold):
    try:
        ...
        if brightness < brightness_threshold:
            ...
            # 确保处理后的图像不是全黑的
            if is_image_too_dark(processed_image):
                print(f"Processed image on page {page_num}, xref {xref} is too dark, skipping this image.")
                return None
...
def process_pdf(input_pdf_path, output_pdf_path, brightness_threshold=75):
    """处理PDF中的较暗图像。"""
    # 打开PDF文件
...
if __name__ == '__main__':
    # 输入和输出PDF文件路径
    input_pdf_path = r"D:\二公司\2023年度决算\2023年4季度\中兴财审补充资料\第二批：二公司\TEST.pdf"
    output_pdf_path = os.path.splitext(input_pdf_path)[0] + "改2.pdf"
